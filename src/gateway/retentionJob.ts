 ...
 *
 * This keeps the Neon DB lean and fast.
 */
...
async function runRetentionCleanup(): Promise<void> {
  ...
  try {
    const purgedCount = await purgeOldMessages(MESSAGE_TTL_HOURS);
    const duration = Date.now() - startTime;
    console.log(`[Retention] Cleanup complete: purged ${purgedCount} messages in ${duration}ms`);
  } catch (error) {
